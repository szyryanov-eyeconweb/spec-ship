...
```jsonc
{
  ...
  "data": [
    ...
    // матрицы, шаблоны строк, справочные списки. [] если нет.
    {
      ...
      // null + заполненный value_ref, когда значение крупное (см. ниже).

      "value_ref": null
      // Вместо инлайн-value, когда значение крупное: матрица на десятки строк,
      // справочный список, длинный шаблон. Значение живёт файлом, в артефакт
      // идёт ДЕСКРИПТОР — так корпус не съедает контекст сабагентов, а точность
      // остаётся проверяемой (checksum строже, чем сверка глазами).
      // {
      //   "path": ".ship/pipeline/{slug}/data/d-1-rakeback-matrix.csv",
      //   "shape": "200 строк × 4 колонки (tier, currency, rate, valid_from)",
      //   "sample": ["1,USD,0.05,2026-01-01", "…", "12,KRW,0.031,2026-01-01"],
      //   "checksum": "sha256:<хеш файла>"
      // }
    }
  ],
...
- `data[]`: ровно одно из `value` / `value_ref` заполнено. Крупное значение (матрица на десятки строк, справочный список, длинный шаблон) → файл в `{slug}/data/` + дескриптор `value_ref`; мелкое (скаляр, короткий список, шаблон в строку) → инлайн `value`. Инлайн крупной матрицы копируется дальше в каждый TaskSpec-носитель и съедает контекст сабагентов; дескриптор — нет. `checksum` обязателен при `value_ref`: он делает «точное значение» проверяемым на каждой границе, а не доверенным.
