...
Если в BusinessDoc есть `data[]` — пробросить в TaskSpec-носитель сценариев subset записей под его сценарии, КОПИЕЙ значений с теми же `d-N` id. Не ссылкой: сабагенты в изолированном контексте, bd не видят — TaskSpec самодостаточен. `test_scenarios[].input` ссылается на `d-N`, не дублирует значение в прозе.
