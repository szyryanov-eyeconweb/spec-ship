...
### Drift-чек: канон не врёт про код (детерминированно)

Канон-доки обещают факты про код (символ существует, enum содержит все значения). Код меняется молча — канон врёт, пока человек не заметит. Drift-чек ловит расхождение машинно, на pre-commit/CI, не дисциплиной LLM.

```
.ship/docs/claims.yaml      контракт: «док X обещает факт про символ/файл Y»
.ship/docs/check_drift.py   stdlib-чекер: сверяет claims с кодом → exit 1 при FAIL
composer analyze-drift      (или pre-commit hook) — гейт

"scripts": {
    "analyze-drift": [
        "Composer\\Config::disableProcessTimeout",
        "python3 .ship/docs/check_drift.py"
    ],
}
```

Типы claim:
- `symbol-exists` — класс/интерфейс/enum/trait, на который ссылается канон, ещё в коде. Ловит переименование/удаление.
- `enum-values` — секция дока про enum содержит ВСЕ backed-значения из кода. Ловит «добавили case в код, забыли в канон».
- `pointer-only` — символ жив в указанном файле; состав не транскрибируется (для часто меняющегося).

Семантика: `exit 1` при любом FAIL. Source-файл отсутствует (другая ветка, код не влит) → SKIP, не fail — канон не блокирует ветки без соответствующего кода. `claims.yaml` — спецификация: чекер переписывается на любом языке из контракта.

Ловит структуру (символ-призрак, забытое enum-значение — самое частое враньё доков), не смысл (неверную workflow-стрелку при живых символах не заметит). Дёшево и детерминированно. Заимствовано из team-os/brain.
