...
3. **Точные данные.** Ставки, лимиты, матрицы, шаблоны — фиксируются в поле `data` с точными значениями. «Комиссия около 5%» — это не данные, это открытый вопрос: агент дожмёт до точного значения или запишет вопрос.
   ...
   Крупные данные (матрица на 200 строк, справочник валют, длинный шаблон) в спеку целиком не кладутся: они ложатся файлом рядом, а в спеке остаётся дескриптор — где файл, сколько в нём строк и колонок, пара строк для чтения глазами, и контрольная сумма. Точность от этого не страдает, а наоборот: сумма ловит изменённую константу надёжнее, чем сверка глазами двухсот строк.
