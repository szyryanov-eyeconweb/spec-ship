...
## Пять проверок (плюс одна для баг-фикса)

1. **spec_coverage** — весь заявленный интерфейс реализован, недокументированных endpoint'ов и параметров нет, точные значения из `data` присутствуют в коде ровно как в спеке (тихо округлённая константа = провал). Крупные данные, лежащие файлом, проверяются контрольной суммой, а не глазами; тест или код, построенный на выдержке из дескриптора вместо файла целиком, — тоже провал.
2. **test_scenarios_covered** — на каждый сценарий из задачи есть тест; тесты идут через публичный интерфейс; если у сценария задан workflow с ожидаемым исходом — тест проверяет ровно его, не ослабленную версию; фикстуры построены из точных значений. Слайс с `test_scenarios: []` валиден — его покрывает функциональный набор фичи на слайсе-носителе; проверяется, что носитель есть и покрывает критерии приёмки слайса.
...
Шестая проверка применяется только к задачам из [bug-fix](09-bug-fix.md), на фича-задачах помечается «n/a»:

6. **regression_guard** — репро-тест был красным до фикса и остался в наборе (не удалён, не в skip); фикс лежит в общей точке, где сходятся все вызывающие, а не в одном пути из тикета; каждый сосед, сломанный тем же корнем, либо покрыт этим билдом, либо имеет свою задачу; чистка испорченных данных идёт отдельной задачей, а не смешана с фиксом кода.

## Вердикты

| Вердикт | Условие | Что дальше |
|---|---|---|
| **APPROVED** | все проверки прошли | `mr_ready: true` — открывайте MR |
...
`review-*.json` — ReviewReport: вердикт, чеклист проверок с заметками, список замечаний (для NEEDS_WORK/ESCALATE), флаг `mr_ready`.
