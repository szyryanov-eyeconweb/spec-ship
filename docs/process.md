...
### Шаг 00. Roadmap — карта эпика ([подробнее](00-roadmap.md)) — только для крупного

Этот шаг нужен, **только если задача крупнее одной фичи**: эпик на несколько фич, скоуп размыт, решения сцеплены («сначала выбрать БД, потом формулировать фичи»). Для одной понятной фичи он пропускается — начинайте с survey.

```
/spec-ship:roadmap Перевести денежные колонки на Decimal256 без overflow слабых валют
```

Агент чертит карту решений `.ship/roadmap/{epic}/MAP.json`: цель эпика, тикеты-вопросы (какие колонки задеты, какой scale, порядок миграции) и **туман** — то, что точно впереди, но ещё не сформулировать. Дальше резолвит тикеты по одному; каждый, что дозрел до «фичу можно делать», уходит в `run` с обогащённым контекстом. Roadmap решает ЧТО и в каком порядке; дальше по каждой фиче работает обычный пайплайн ниже.

Наш пример — одна фича, поэтому roadmap не нужен: сразу survey.
...
### Развилка: а если это баг? ([подробнее](09-bug-fix.md))

Дальше по этой странице идёт путь **фичи** — когда известно, что строить. Если вход другой — «сломано вот тут» (падающий тест, не та сумма, регрессия), — два следующих шага заменяются одним:

```
/spec-ship:bug-fix Партнёрский баланс уходит в минус после отмены заказа; репро OrderCancelTest
```

Причина: интервью «какое поведение хотим?» на баге даёт ответ «правильное» (целевое поведение у бага не придумывают, а выясняют — из workflow-доков и ADR), а резать на слайсы нечего, когда корень один. `bug-fix` вместо этого пишет репро-тест **до** анализа, ищет корень (а не место, где симптом заметен), проверяет, кого ещё этот корень ломает, — и отдаёт обычный TaskSpec. Начиная с шага 2 всё идёт как ниже.
...
Агент режет фичу на **вертикальные слайсы** — маленькие задачи, каждая проходит через все слои (схема → логика → API) и собирается независимо. Тесты идут на фичу, не на слайс: один функциональный набор на всю фичу (на слайсе внешнего входа) + unit только там, где есть нетривиальная бизнес-логика. Слайс без своей логики собственных тестов не несёт. Каждой задаче назначается **зона доверия**:
...
Результат — несколько **TaskSpec** (`task-*.json`): у каждой задачи интерфейс, список файлов, которые можно менять; сценарии тестов — на слайсе-носителе (функц. набор фичи или unit на логику), прочие слайсы — с пустым набором.
...
1. **RED** получает сценарии тестов из TaskSpec и пишет по падающему тесту на каждый. Он читает `src/`, но писать может только в `tests/` — тест честный, под несуществующую реализацию его не подгонишь. Слайс без своих сценариев RED пропускает (покрыт функц. набором фичи), GREEN реализует его напрямую.
...
Пять проверок: интерфейс реализован полностью; каждый сценарий покрыт тестом (и тест проверяет ровно заявленный результат, не ослабленный); ни один ADR не нарушен; нет регрессий; performance-требования из Definition of Done адресованы. Для задач из [bug-fix](09-bug-fix.md) добавляется шестая — репро-тест был красным до фикса и остался в наборе, фикс лежит в общей точке, сломанные соседи не забыты.
...
## Какой сценарий у моей задачи

Не всякая задача требует полного цикла. Как гнать экономично для клона, простой фичи, дельты, исследования — [cases.md](cases.md).
