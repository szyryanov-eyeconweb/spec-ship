...
```
/spec-ship:bug_fix Партнёрский баланс уходит в минус после отмены заказа; репро OrderCancelTest
```

Причина: интервью «какое поведение хотим?» на баге даёт ответ «правильное» (целевое поведение у бага не придумывают, а выясняют — из workflow-доков и ADR), а резать на слайсы нечего, когда корень один. `bug_fix` вместо этого пишет репро-тест **до** анализа, ищет корень (а не место, где симптом заметен), проверяет, кого ещё этот корень ломает, — и отдаёт обычный TaskSpec. Начиная с шага 2 всё идёт как ниже.
