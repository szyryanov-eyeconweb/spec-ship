...
**Починка бага** — особый случай: там survey не опционален, а обязателен, и вызывает его [bug_fix](09-bug-fix.md) сам. Якорь у бага двухчастный: точка, где симптом **наблюдается**, при том что корень может быть в другом месте. Survey его не объявляет — только собирает карту; диагноз ставит bug_fix.
...
- **doc-promote** — после мёржа наблюдения станут частью канона поведения (а в режиме backfill survey существующего кода можно промоутнуть в канон сразу, без фичи)
