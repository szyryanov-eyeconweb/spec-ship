...
**Когда пропустить:** одна понятная фича на 3–15 слайсов → сразу [`/spec-ship:run`](08-run.md). Баг → [`/spec-ship:bug-fix`](09-bug-fix.md) (своя ветка, карта не нужна). Опечатка, бамп версии → без пайплайна вовсе. Скоуп ясен, порядок ясен, архитектура известна → карта = оверхед.
