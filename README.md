...
[3]  review      — проверка по чеклисту → вердикт         (человек нужен только при эскалации)
   ▼
merge request → после мёржа: adr-promote / doc-promote — знание в канон
```

Команды в Claude Code: `/spec-ship:survey`, `/spec-ship:shape-doc`, `/spec-ship:decompose`, `/spec-ship:build`, `/spec-ship:review`, `/spec-ship:adr-promote`, `/spec-ship:doc-promote`.
...
| [Delivery: doc-promote](docs/07-doc-promote.md) | Поведение — в канон workflow-доков |
...
# 2. В Claude Code внутри проекта:
...
```
skills/      7 скиллов пайплайна + README-канон + протокол ADR-CONFLICT
